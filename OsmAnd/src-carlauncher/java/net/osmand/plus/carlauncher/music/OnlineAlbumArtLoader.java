...
public final class OnlineAlbumArtLoader {
    ...
    private static final String USER_AGENT = "OsmAnd-Auto/1.0 (https://github.com/poolsoft/OsmAnd)";
    ...
    public static OnlineAlbumArtLoader getInstance(Context context) {
        if (instance == null) {
            synchronized (OnlineAlbumArtLoader.class) {
    ...
    public void load(String title, String artist, Callback callback) {
        String policy = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
                .getString(POLICY_KEY, "off");
    ...
    private boolean networkAllowed(String policy) {
        ...
        return "any".equals(policy) || !cm.isActiveNetworkMetered();
    }
    ...
    private Bitmap downloadCover(String groupId) throws Exception {
        JSONObject root = new JSONObject(readText("https://coverartarchive.org/release-group/" + groupId + "/"));
