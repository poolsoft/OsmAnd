...
public class QuickSearchListAdapter extends ArrayAdapter<QuickSearchListItem> {
	...
	private String lastAutoSearchPhrase = "";
	...
	private LinearLayout bindSearchMoreItem(@Nullable View convertView,
		...
		// Auto expand logic
		if (emptySearchVisible && searchMoreItem.isSearchMoreAvailable()) {
			String currentText = searchPhrase.getText(true);
			if (currentText != null) {
				if (!currentText.equals(lastAutoSearchPhrase)) {
					lastAutoSearchPhrase = currentText;
					autoSearchCount = 0;
				}
